...
/* max length of a path is set to 1024 (change if needed) - PATH_MAX is of 4096 */
#define MAX_PATH_LEN 1024
/* max length of a file name is set to 256 (change if needed) */
#define MAX_FILENAME_LEN 256
/* max length of a encryption key is set to 20 (change if needed) */
#define MAX_ENCKEY_LEN 20
...
/**
* @brief Custom struct created for copy functionaly
* @param infile path of old file
* @param outfile path of new file
* @param keybuf encryption key
* @param keylen encryption key length
* @param infile_size total file size
*/
typedef struct Cryptocopy_Params{
	char *infile;
	char *outfile;
	char *keybuf;
	int keylen;
	unsigned long long infile_size;
}cryptocopy_params;
