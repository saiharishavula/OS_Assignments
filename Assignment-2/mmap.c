...
static vm_fault_t stbfs_fault(struct vm_fault *vmf)
{
	vm_fault_t err;
	...
	lower_vm_ops = STBFS_F(file)->lower_vm_ops;
	BUG_ON(!lower_vm_ops);
	 ...
	 * lower file, a concurrent invocation of stbfs_fault could see a
...
static vm_fault_t stbfs_page_mkwrite(struct vm_fault *vmf)
{
	vm_fault_t err = 0;
        struct vm_area_struct *vma = vmf->vma;
	struct file *file, *lower_file;
	 ...
	 * invocation of stbfs_page_mkwrite could see a different
...
static ssize_t stbfs_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
	/*
...
const struct address_space_operations stbfs_aops = {
	.direct_IO = stbfs_direct_IO,
};

const struct vm_operations_struct stbfs_vm_ops = {
	.fault		= stbfs_fault,
	.page_mkwrite	= stbfs_page_mkwrite,
};
